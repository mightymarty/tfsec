...
type adapter struct {
	...
	bucketMap map[string]*s32.Bucket
}

func (a *adapter) adaptBuckets() []s32.Bucket {
...
func getEncryption(block *terraform2.Block, a *adapter) s32.Encryption {
	if block.HasChild("server_side_encryption_configuration") {
		return s32.Encryption{
			Metadata:  block.GetMetadata(),
...
func getVersioning(block *terraform2.Block, a *adapter) s32.Versioning {
	if block.HasChild("versioning") {
		return s32.Versioning{
			Metadata: block.GetMetadata(),
...
func getLogging(block *terraform2.Block, a *adapter) s32.Logging {
...
func getBucketAcl(block *terraform2.Block, a *adapter) types2.StringValue {
...
func isEncrypted(encryptionBlock *terraform2.Block) types2.BoolValue {
...
func hasLogging(b *terraform2.Block) types2.BoolValue {
...
func isVersioned(b *terraform2.Block) types2.BoolValue {
	...
	if versioningBlock := b.GetBlock("versioning_configuration"); versioningBlock.IsNotNil() {
		...
		if status.Equals("Enabled", terraform2.IgnoreCase) {
