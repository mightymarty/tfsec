...
type wrappedDocument struct {
	Source   scan2.MetadataProvider
	Document iamgo.Document
}

func ParsePolicyFromAttr(attr *terraform2.Attribute, owner *terraform2.Block, modules terraform2.Modules) (*iam2.Document, error) {

	documents := findAllPolicies(modules, owner, attr)
	if len(documents) > 0 {
		return &iam2.Document{
...
func ConvertTerraformDocument(modules terraform2.Modules, block *terraform2.Block) (*wrappedDocument, error) {
...
//nolint
func parseStatement(statementBlock *terraform2.Block) iamgo.Statement {
...
func findAllPolicies(modules terraform2.Modules, parentBlock *terraform2.Block, attr *terraform2.Attribute) []wrappedDocument {
	...
	for _, ref := range attr.AllReferences() {
		for _, b := range modules.GetBlocks() {
			...
			kref := *ref
			kref.SetKey(parentBlock.GetMetadata().Reference().(*terraform2.Reference).RawKey())
